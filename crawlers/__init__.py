...
def get_text_of_all_pdfs_linked_from(url):
    ...
    docs = [{'url': urlparse.urljoin(urlparse.urlparse(url)['netloc'], link['href']) if link['href'].startswith('/') else urlparse.urljoin(url, link['href']), 'tag': link.contents[0]} for link in soup.find_all('a', href=True) if link['href'].endswith('.pdf')]
    for doc in docs:
