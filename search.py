    ...
    if 'action' in payload:
        ...
        for field in results['fields']:
            if field == 'id':
                ...
                items = sorted(list(set([row[field] if isinstance(row[field], basestring) else '' for row in results['data'] if field in row and len(unicode(row.get(field)).encode('utf-8')) < 20])))
